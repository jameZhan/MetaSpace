...
contract MetaSpace is Context, Ownable, MinterRole, Pausable, ERC721, IMetaSpace {
    ...
    uint256 private _level2Area = 4;
    uint256 private _level3Area = 9;
    uint256 private _level4Area = 16;
    uint256 private _level5Area = 36;
    uint256 private _level6Area = 100;
    // base values for luxury of 6 levels
    // notice: /100 to show on the frontend
    ...
    // notice: /1000 to show on the frontend
    ...
    event SetHouseAttributes(uint256 indexed tokenId, uint256 indexed level, uint256 buildingPeriod, 
                             uint256 area, uint256 luxury, uint256 durability, uint256 luck, uint256 rentingPeriod,
                             uint256 protectingPeriod);

    constructor() ERC721("Meta Space", "METAS") public {

    }

    function safeMint(address to, uint256 level) public virtual override onlyMinter {
        require(level >= 1 && level <=6, "level is not in the range of [1, 6]");

        // start from #1
        _currentId.increment();
        uint256 tokenId = _currentId.current();
        // set random attributes first, then mint; mint will invoke onERC721Received();
    ...
    function checkHouseAttributes(uint256 tokenId) public view returns (attributes memory) {
    ...
    function _setHouseAttributes(uint256 tokenId, uint256 level) private {
        ...
        emit SetHouseAttributes(tokenId, level, thisHouse.buildingPeriod, 
                                thisHouse.area, thisHouse.luxury, thisHouse.durability, 
                                thisHouse.luck, thisHouse.rentingPeriod, thisHouse.protectingPeriod);
    }
