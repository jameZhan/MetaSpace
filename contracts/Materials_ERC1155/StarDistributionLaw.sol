...
contract StarDistrbutionLaw is Context, Ownable, ReentrancyGuard{
    ...
    // record each round's total Star reward balance; reset when the next round burn time starts
    ...
    // landId(lid) => total burned mateials&star points in this period for this land; from 1 to 7
    ...
    event AddPoints(address indexed account, uint256 indexed lid, uint256 indexed amount);
    event InjectPoints(address indexed account, uint256 indexed lid, uint256 indexed amount);
    event ClaimStarRewards(address indexed account, uint256 indexed lid, uint256 indexed rewards);
    ...
    function burnMaterialsForLand(uint256 lid, uint256[] memory materialIds, uint256[] memory materialAmounts) public {
        ...
        emit AddPoints(_msgSender(), lid, addPoints);
    }

    // user MUST approve before use this function
    function burnStarForLand(uint256 lid, uint256 amount) public {
        require(isBurnTime, "Burn Time has NOT arrived!");
        require(lid > 0 && lid <=7, "land id is out of range of [1, 7]");

        rewardsToken.burnFrom(_msgSender(), amount);
        // accumulate points for user and land
        uint256 addPoints = amount.mul(300).div(1e18);                                           // decimal: 1e0
        _storedPointsOf[_msgSender()][lid] = _storedPointsOf[_msgSender()][lid].add(addPoints); // decimal: 1e0
        _totalPointsOf[lid] = _totalPointsOf[lid].add(addPoints);                               // decimal: 1e0
    ...
    function claimStarRewards(uint256 lid) public nonReentrant {
        ...
        emit ClaimStarRewards(_msgSender(), lid, rewards);
    }
    ...
    function _setMaterialPoints() private {
        _materialPoints[1] = 3;    // id=1 
        _materialPoints[2] = 4;    // id=2 
        _materialPoints[3] = 15;   // id=3 
        _materialPoints[4] = 15;   // id=4 
        _materialPoints[5] = 15;   // id=5 
        _materialPoints[6] = 6;    // id=6 
        _materialPoints[7] = 10;   // id=7 
        _materialPoints[8] = 50;   // id=8 
        _materialPoints[9] = 75;   // id=9 
        _materialPoints[10] = 75;  // id=10 
    }
